...
/* 
Main point of all those code lines below is to configure list of external 
...
class CommandRank : public Command {
  int local_address;
  public: 
  CommandRank (char * _name, int rank): Command(_name){
    local_address = rank;
    if (local_address == 0){
      Wire.begin();
      Wire.onReceive(NULL);}
    else{
      Wire.begin(local_address);
  ...
  void dump(char * str){
    sprintf(str, "rank %s %d", name, local_address);
  };
};


class CommandRemoteI2C : public Command {
  int remote_address;
  char * remote_name;
  public: 
  CommandRemoteI2C (char * _name, int _remote_address, char * _remote_name): Command(_name){
    remote_address = _remote_address;
    remote_name = (char *) malloc(strlen(_remote_name) + 1);
  ...
  void dump(char * str){
    sprintf(str, "i2c %s %d", name, remote_address);
  };
};
...
void do_stat(){
    ...
    check_mem();
    Serial.println("------------");
    Serial.println((int)builtins);
    Serial.println((int)commands);
    Serial.println((int)last_malloc);
    Serial.println((int)max_malloc);
    Serial.println((int)heapptr);
    Serial.println((int)stackptr);
    for (int i = 0; i< BUILTIN_COUNT;i++){
      Serial.println((int)builtins[i]);
      Serial.println(builtins[i]->name);
    }
...
public :
  CommandList(char * name) : Command(name) {};
  virtual void action (const char * arg){
...
public:
  ...
  void action (const char * arg){
    ...
    if (strcmp(name, "i2c") == 0){
      end1 = getParam(name, arg, end1);
      char remote_address [MAX_CMD_LEN];
      end1 = getParam(remote_address, arg, end1);
      char remote_name[MAX_CMD_LEN];
      end1 = getParam(remote_name, arg, end1);
      cmd = new CommandRemoteI2C(name, atoi(remote_address), remote_name);
    }
     else
    if (strcmp(name, "rank") == 0){
      end1 = getParam(name, arg, end1);
      char local_address [MAX_CMD_LEN];
      end1 = getParam(local_address, arg, end1);
      cmd = new CommandRank(name, atoi(local_address));
    };   
...
public :
  CommandStat(char * _name): Command(_name){
  };
  void action(const char * arg){
    do_stat();
  };
...
*/ 
  ...
  {
    i2c_readln(cmd); // receive byte as a character
...
void setup() {
  ...
  cotton::builtins[4] = new cotton::CommandStat("stat");
  cotton::builtins[5] = new cotton::CommandList("list");
